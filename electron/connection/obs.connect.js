...
import { detachOpsLog } from "../services/obs.opslog.service.js";
...
function setState(next) {
  if (state !== next) {
    ...
    state = next;
    log(
      `${COLOR.cyan}[OBS] state${COLOR.reset} ${prettyState(
        prev
      )} → ${prettyState(next)}`
    );
...
function setReady(val) {
  if (ready !== !!val) {
    ready = !!val;
    log(
      `${COLOR.cyan}[OBS] ready${COLOR.reset} → ${
        ready ? COLOR.green + "true" : COLOR.yellow + "false"
      }${COLOR.reset}`
    );
...
export function onSceneList(fn) {
  events.on("scene-list", fn);
  try {
    fn([]);
  } catch {}
  return () => events.off("scene-list", fn);
}
/* =========================
...
function attachLifecycleListeners() {
  ...
  obs.on("ConnectionClosed", (info) => {
    ...
    console.warn(
      `${COLOR.red}[OBS] Connection closed${COLOR.reset}`,
      info || ""
    );
  });
  ...
  obs.on("error", (e) => {
    console.warn(
      `${COLOR.yellow}[OBS] Client error${COLOR.reset}:`,
      e?.message || e
    );
    if (connected) {
      setState("unstable");
  ...
  obs.on("CurrentProgramSceneChanged", (p) =>
    log("[OBS] Program scene →", p?.sceneName)
  );
  obs.on("CurrentSceneCollectionChanged", (p) =>
    log("[OBS] Scene collection →", p?.sceneCollectionName)
  );
  obs.on("SceneCollectionListChanged", (p) =>
    log("[OBS] Scene collection list changed:", p)
  );
}
...
export async function connect({ url = "ws://127.0.0.1:4455", password } = {}) {
  if (connected) {
    log(
      `${COLOR.cyan}[OBS] connect called but already connected${COLOR.reset}`
    );
    return { status: "ok", alreadyConnected: true };
  }

  log(
    `${COLOR.cyan}[OBS] connecting to${COLOR.reset} ${url}  pwd? ${
      password ? "yes" : "no"
    }`
  );
  try {
    ...
    log(`${COLOR.green}✅ [OBS] connected${COLOR.reset}`, {
      obsWebSocketVersion,
    ...
    setState("disconnected");
    console.error(
      `${COLOR.red}❌ [OBS] connect failed${COLOR.reset}:`,
      err?.message || err
    );
    throw err;
  }
...
export async function disconnect() {
  try {
    ...
    log(
      `${COLOR.yellow}[OBS] disconnect error (ignored)${COLOR.reset}:`,
      e?.message || e
    );
  }finally {
    try { detachOpsLog(obs); } catch {}
  }
  connected = false;
  setState("disconnected");
